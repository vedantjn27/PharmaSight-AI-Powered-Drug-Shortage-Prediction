...
import { ArrowRight, Zap, TrendingDown, BarChart3, Sliders, Brain, Network, ShoppingCart, CheckCircle } from 'lucide-react'
...
export default function LandingPage() {
  const { setBackendHealthy } = useAppStore()
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[
            ...
            },
            {
              ...
              description: 'AI-optimized purchase orders and inventory management',
              icon: '⚙️',
            },
          ].map((item, i) => (
            <motion.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6, delay: i * 0.1 }}
              className="glass p-8 rounded-xl border border-primary-500/20"
            >
              <div className="text-4xl mb-4">{item.icon}</div>
              <h3 className="text-xl font-bold mb-3">{item.title}</h3>
              <p className="text-muted">{item.description}</p>
